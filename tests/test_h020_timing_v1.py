...
from marketlab.h020_timing_v1 import compute_snapshot
...
def test_future_rows_do_not_change_asof_snapshot_when_excluded() -> None:
    sessions = 260
    benchmark = _frame(
        100.0 + np.arange(sessions) * 0.04 + np.sin(np.arange(sessions) / 7)
    )
    stock = _frame(
        100.0 + np.arange(sessions) * 0.06 + 3 * np.sin(np.arange(sessions) / 8)
    )
    base = compute_snapshot(stock, benchmark, symbol="POINTINTIME")

    future_index = pd.bdate_range(
        stock.index[-1] + pd.Timedelta(days=1), periods=20, tz="UTC"
    )
    ...
    replay = compute_snapshot(
        combined_stock, combined_benchmark, symbol="POINTINTIME"
    )

    assert replay["action"] == base["action"]
    assert (
        replay["timing_score_0_100_not_probability"]
        == base["timing_score_0_100_not_probability"]
    )
