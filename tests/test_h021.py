...
def test_validate_snapshot_rejects_future_source_date() -> None:
    snap = _snapshot("2026-09-11")
    snap["observations"][0]["source_observed_market_date"] = "2026-09-12"
    assert any("after capture date" in err for err in validate_snapshot(snap))


def test_validate_snapshot_rejects_duplicate_symbol_period() -> None:
    snap = _snapshot("2026-09-11")
    snap["observations"].append(copy.deepcopy(snap["observations"][0]))
    assert any("duplicate observation key" in err for err in validate_snapshot(snap))
