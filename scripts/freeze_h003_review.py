...
def build_summary(ledger) -> dict[str, Any]:
    ...
    metrics = dict(
        sorted(Counter(claim.metric for claim in ledger.accepted_claims).items())
    )
    claim_types = dict(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Freeze a complete H003-V001 review ledger and accepted ManagementClaims."
    )
    ...
    ledger = freeze_review_ledger(corpus, list(decisions), list(blind_payloads))
