...
<xbrli:xbrl xmlns:xbrli="http://www.xbrl.org/2003/instance"
 ...
 <co:SecuritiesAcquiredOrDisposedNumberOfSecurity contextRef="Disclosure1" unitRef="shares">{quantity}</co:SecuritiesAcquiredOrDisposedNumberOfSecurity>
 ...
 <co:ExchangeOnWhichTheTradeWasExecuted contextRef="Disclosure1">{exchange}</co:ExchangeOnWhichTheTradeWasExecuted>
...
def test_non_discretionary_or_non_direct_transactions_do_not_qualify() -> None:
...
def test_zero_quantity_value_and_non_exchange_rows_do_not_qualify() -> None:
    assert not parse_pit_xml(_xml(quantity="0")).transactions[0].is_direct_market_purchase
...
def test_parser_contract_freezes_units_and_candidate_semantics() -> None:
    ...
    assert contract["primary_candidate_quantity_rule"] == "strictly_positive"
    assert contract["primary_candidate_value_rule"] == "strictly_positive_INR"
    assert contract["primary_candidate_exchanges"] == ["BSE", "NSE"]
