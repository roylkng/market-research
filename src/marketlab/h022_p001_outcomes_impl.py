...
def load_reviewed_sessions(
    ...
    for index, row in enumerate(raw_sessions):
        ...
        if (
            opened.astimezone(IST).date() != session_date
            or closed.astimezone(IST).date() != session_date
        ):
            raise H022P001OutcomeError(
                f"session {session_date} timestamps do not map to India date"
            )
...
def _unresolved_between(
    unresolved: tuple[date, ...], *, start_date: date, end_date: date
) -> tuple[str, ...]:
...
def _cutoff_session(
    sessions: tuple[HistoricalSession, ...], cutoff: date
) -> HistoricalSession:
    matches = [session for session in sessions if session.session_date == cutoff.isoformat()]
    if len(matches) != 1:
        raise H022P001OutcomeError(
            "market-data cutoff must be an exact reviewed completed NSE session"
        )
    return matches[0]
...
def build_prospective_outcome_report(
    ...
    *,
    ...
    evaluation_frozen = _timestamp(
        evaluation_frozen_at_utc, field="evaluation_frozen_at_utc"
    )
    ...
    cutoff_session = _cutoff_session(sessions, cutoff)
    if _timestamp(
        cutoff_session.close_timestamp_utc, field="market_data_cutoff_close"
    ) > evaluation_frozen:
        raise H022P001OutcomeError(
            "market-data cutoff session had not completed by evaluation freeze"
        )
    ...
    report: dict[str, Any] = {
        ...
        "evaluation_frozen_at_utc": _utc_text(evaluation_frozen),
...
def summarize_prospective_outcomes(report: dict[str, Any]) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        ...
        "evaluation_frozen_at_utc": report["evaluation_frozen_at_utc"],
        "primary_horizon_sessions": PRIMARY_HORIZON,
