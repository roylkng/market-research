...
COMPILER_REVISION = "H002-HR002-phase-a-r2-six-quarter-mixed-source"
...
def _fetch_discovery_for_variants(
    ...
    for symbol in historical_symbol_variants(member.symbol):
        ...
        try:
            integrated_payload, integrated_raw = client.integrated_financial_filings_with_raw(
                symbol
            )
        except NSEAcquisitionError as exc:
            evidence["integrated"].append(
                {"symbol": symbol, "status": "FETCH_FAILED", "error": str(exc)}
            )
        else:
            retained = _retain(
                store_root,
            ...
            retained.update(
                {"symbol": symbol, "source_url": client.INTEGRATED_FILING_ENDPOINT.url}
            )
...
def _freeze_discovery_manifest(
    store_root: Path,
    *,
    member: UniverseMember,
    evidence: dict[str, Any],
    ...
    artifact = _retain(
        store_root,
        raw,
        kind="mixed-discovery-manifests",
        suffix=".json",
    )
    return {**evidence, "composite": artifact}
...
def _select_pair(
    *,
    ...
    for accounting_basis in ("Consolidated", "Standalone"):
        ...
        if len(first_urls) != 1:
            raise PhaseAV2Error(
                f"ambiguous first target filing for {member.symbol}/{quarter['id']}: "
                f"{sorted(first_urls)}"
            )
        ...
        if len(latest_urls) != 1:
            raise PhaseAV2Error(
                f"ambiguous baseline at freeze for {member.symbol}/{quarter['id']}: "
                f"{sorted(latest_urls)}"
            )
        target = first_rows[-1]
...
def _validate_2025_calendar_sources(holiday_raw: bytes, muhurat_raw: bytes) -> None:
    try:
        holiday_text = "\n".join(
            page.extract_text() or "" for page in PdfReader(io.BytesIO(holiday_raw)).pages
        )
        muhurat_text = "\n".join(
            page.extract_text() or "" for page in PdfReader(io.BytesIO(muhurat_raw)).pages
        )
...
def _process_pair(
    *,
    ...
    composite = discovery_evidence.get("composite")
    if not isinstance(composite, dict):
        raise PhaseAV2Error("frozen mixed-discovery artifact is missing")
    ...
    pre_event_basis = audit_price_basis_actions(
        ...
        raw_payload=action_raw,
        symbol=member.symbol,
        start_date=freeze.astimezone(IST).date(),
        end_date=publication.astimezone(IST).date(),
    )
    if (
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for index, member in enumerate(members, start=1):
        try:
            integrated_payload, legacy_payload, raw_discovery_evidence = (
                _fetch_discovery_for_variants(client, store_root, member)
            )
            discovery_evidence = _freeze_discovery_manifest(
                store_root,
                member=member,
                evidence=raw_discovery_evidence,
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Capture H002-HR002 six-quarter signals without outcomes"
    )
    ...
    parser.add_argument(
        "--output",
        default=(
            "research/historical/h002/H002-HR002/phase-a/"
            "fixed-u001-six-quarter-signals.json"
        ),
    )
