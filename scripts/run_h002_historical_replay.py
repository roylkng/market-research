...
def _assert_event_identity(
    ...
    *,
    ...
    if event.symbol.upper() != member.symbol.upper():
        raise PhaseAError(
            f"{role} symbol mismatch: expected={member.symbol}, observed={event.symbol}"
        )
    if member.isin and event.isin and member.isin != event.isin:
        raise PhaseAError(
            f"{role} ISIN mismatch for {member.symbol}: "
            f"expected={member.isin}, observed={event.isin}"
        )
    observed_period = _period_iso(event.reporting_period_end)
    if observed_period != expected_period_end:
        raise PhaseAError(
            f"{role} period mismatch for {member.symbol}: "
            f"expected={expected_period_end}, observed={observed_period}"
        )
    if event.accounting_basis.strip().casefold() != expected_basis.strip().casefold():
        raise PhaseAError(
            f"{role} accounting-basis mismatch for {member.symbol}: "
...
def run_phase_a(args: argparse.Namespace) -> dict[str, Any]:
    ...
    holiday_evidence = _retain(
        store_root, holiday_raw, kind="holiday-master", suffix=".json"
    )
